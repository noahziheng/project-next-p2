...
Baby.prototype.init = function () {
  ...
  this.angle = 0
  var i = 0
  for (i = 0; i < 8; i++) { // 初始化小鱼的尾巴图片数组
    this.babyTailArr[i] = new window.Image()
    this.babyTailArr[i].src = imgUrl + 'babyTail' + i + '.png'
  }
  for (i = 0; i < 2; i++) { // 初始化小鱼的眼睛图片数组
    this.babyEyeArr[i] = new window.Image()
    this.babyEyeArr[i].src = imgUrl + 'babyEye' + i + '.png'
  }
  for (i = 0; i < 20; i++) { // 初始化小鱼的身体图片数组
    this.babyBodyArr[i] = new window.Image()
