...
Mom.prototype.init = function () {
  ...
  var i = 0
  for (i = 0; i < 8; i++) { // 大鱼尾巴
    this.momTailArr[i] = new window.Image()
    this.momTailArr[i].src = imgUrl + 'bigTail' + i + '.png'
  }
  for (i = 0; i < 2; i++) { // 大鱼眼睛
    this.momEyeArr[i] = new window.Image()
    this.momEyeArr[i].src = imgUrl + 'bigEye' + i + '.png'
  }
  for (i = 0; i < 8; i++) {
    this.momOrangeArr[i] = new window.Image() // 大鱼橙色身体
    this.momOrangeArr[i].src = imgUrl + 'bigSwim' + i + '.png'
    this.momBlueArr[i] = new window.Image() // 大鱼蓝色身体
